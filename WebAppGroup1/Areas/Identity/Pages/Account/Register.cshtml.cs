...
namespace WebAppGroup1.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        private Spartan CreateUser()
        {
            try
            {
                return Activator.CreateInstance<Spartan>();
            }
            catch
            {
                throw new InvalidOperationException($"Can't create an instance of '{nameof(Spartan)}'. " +
                    $"Ensure that '{nameof(Spartan)}' is not an abstract class and has a parameterless constructor, or alternatively " +
                    $"override the register page in /Areas/Identity/Pages/Account/Register.cshtml");
            }
        }

        private IUserEmailStore<Spartan> GetEmailStore()
        {
            if (!_userManager.SupportsUserEmail)
            {
                throw new NotSupportedException("The default UI requires a user store with email support.");
            }
            return (IUserEmailStore<Spartan>)_userStore;
        }
    }
}
